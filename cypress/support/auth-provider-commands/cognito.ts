...
Amplify.configure({
  Auth: {
    Cognito: {
      userPoolClientId: Cypress.env('COGNITO_CLIENT_ID'),
      userPoolId: Cypress.env('COGNITO_USER_POOL_ID'),
      loginWith: {
        oauth: {
          domain: Cypress.env('COGNITO_DOMAIN'),
          scopes: ['openid', 'email'],
          redirectSignIn: [Cypress.env('COGNITO_REDIRECT_URI')],
          redirectSignOut: [Cypress.env('COGNITO_REDIRECT_URI')],
          responseType: 'code',
        },
        email: true,
        username: true,
...
Cypress.Commands.add(
  ...
  (username: string, password: string) => {
    cy.intercept('POST', /^\/api\/auth\/ensure-user\/?.*$/, (req) => {
      req.reply({
        status: 200,
        body: {
          id: '1',
